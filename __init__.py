...
# Per-host Ed25519 signing for Supabase calls. Imported lazily through both
# package-relative and sys.path shapes so the plugin works whether Hermes
# loads it as `hermes_plugins.onepilot.__init__` (namespace) or as a flat
# module on sys.path.
try:
    from .pop_keys import sign_auth_header  # type: ignore[import-not-found]
except (ImportError, ValueError):
    import sys
    _here = str(Path(__file__).resolve().parent)
    if _here not in sys.path:
        sys.path.insert(0, _here)
    from pop_keys import sign_auth_header  # type: ignore[no-redef]
...
# Reference to the plugin's asyncio loop, set by `_thread_main` once the loop
# is created. The synchronous `pre_tool_call` hook (running on the agent
# thread) uses this to schedule async broadcasts via
# `asyncio.run_coroutine_threadsafe`. None until the daemon thread comes up.
_PLUGIN_LOOP: Optional[asyncio.AbstractEventLoop] = None

# session_key → session_id cache. Populated from inbound user-message rows in
# `_handle_user_message` so the synchronous approval gate can broadcast on the
# correct Realtime topic without a backend round-trip. `_LATEST_SESSION_ID`
# is the most-recent fallback for the open-tool-on-deadlock case (mirror of
# OpenClaw's `lookupAnySessionForAccount`).
_SESSION_ID_CACHE: dict[tuple[str, str], str] = {}
_LATEST_SESSION_ID: dict[str, str] = {}
_SESSION_ID_LOCK = threading.Lock()

# Match `/approve <id> <decision>` before the LLM ever sees it. The `[\w:-]+`
# char class accepts UUIDs, short codes, and the `plugin:<uuid>` shape (the
# colon is intentional — locked by CI symbol-grep so a future regex narrowing
# can't silently break upstream-ID resolution).
APPROVE_REGEX = re.compile(r"^\s*/approve\s+([\w:-]+)\s+(allow-once|allow-always|deny)\b", re.IGNORECASE)


def _remember_session_id(account_id: str, session_key: str, session_id: str) -> None:
    with _SESSION_ID_LOCK:
        _SESSION_ID_CACHE[(account_id, session_key or "main")] = session_id
        _LATEST_SESSION_ID[account_id] = session_id


def _lookup_session_id(account_id: str, session_key: str = "main") -> Optional[str]:
    with _SESSION_ID_LOCK:
        sid = _SESSION_ID_CACHE.get((account_id, session_key))
        if sid:
            return sid
        return _LATEST_SESSION_ID.get(account_id)
...
def register(ctx) -> None:
    ...
    # v0.10.0: agentKey removed; the per-host Ed25519 keypair lives outside
    # config.json. A stale agentKey field is ignored (logged once below).
    required = {
        "backendUrl", "streamUrl", "publishableKey",
    ...
    if "agentKey" in config:
        logger.warning("[onepilot] legacy 'agentKey' field ignored — re-pair via iOS to provision Ed25519 keypair")

    _install_cron_channel(config)
    _start_wrapper_api(config)

    # Hand-off: capture the plugin's asyncio loop reference into _PLUGIN_LOOP
    # before we start scheduling work onto it from foreign threads. The agent
    # thread (where `pre_tool_call` fires) needs this reference to schedule
    # async broadcasts via `run_coroutine_threadsafe`. Without the handshake
    # we'd race: the hook could fire before the loop exists and crash on a
    # None loop reference.
    ...
    if not loop_ready.wait(timeout=5.0):
        logger.error(
            "[onepilot] plugin asyncio loop never came up — approval gate will be inert"
        )
    else:
        _register_approval_gate(ctx, config)
...
def _start_wrapper_api(config: dict[str, Any]) -> None:
    """Start the plugin's HTTP wrapper API on 127.0.0.1:gatewayPort+1.

    Routes (`/onepilot/v1/health|configure|account/*|plugin/uninstall|
    approvals/config`) are bearer-gated against the plugin's wrapperApiToken.
    The iOS adapter calls these to flip the approvals toggle (and other
    config writes) without SSH-shelling `hermes <verb>` — see the plugin
    CLAUDE.md "iOS adapter is not the bridge" rule.

    Best-effort: if `wrapper_api` import fails or the port is occupied,
    log a warning and continue. The plugin's chat path doesn't depend on
    the wrapper, so this must never block startup.
    """
    try:
        # Hermes loads this file as hermes_plugins.onepilot.__init__, so a
        # bare `import wrapper_api` would resolve against the global sys.path
        # (where the plugin dir is NOT). Fall back through both shapes so the
        # import works whether Hermes set up the plugin as a regular package
        # or a namespace import.
        try:
            from .wrapper_api import start_wrapper_api  # type: ignore[import-not-found]
        except (ImportError, ValueError):
            import sys
            here = str(PLUGIN_DIR)
            if here not in sys.path:
                sys.path.insert(0, here)
            from wrapper_api import start_wrapper_api  # type: ignore[no-redef]
    except Exception as exc:
        logger.warning("[onepilot] wrapper_api import failed (%s) — wrapper API disabled", exc)
        return
    # Hermes' api_server platform owns the gateway port. The wrapper binds
    # at gateway_port + 1 (matching iOS's OnepilotPluginClient default).
    # `ONEPILOT_WRAPPER_PORT` env var, if set, takes precedence inside
    # start_wrapper_api so test harnesses can pin a free port.
    try:
        gateway_port = int(os.environ.get("API_SERVER_PORT", "8642"))
    except ValueError:
        gateway_port = 8642
    try:
        srv = start_wrapper_api(initial_config=config, gateway_port=gateway_port)
        if srv is not None:
            logger.info("[onepilot] wrapper API listening on port %d", gateway_port + 1)
        else:
            logger.warning(
                "[onepilot] wrapper API did not start (port %d may be busy)",
                gateway_port + 1,
            )
    except Exception as exc:
        logger.warning(
            "[onepilot] wrapper API failed to start (%s) — iOS approval toggle will fail",
            exc,
        )


def _register_approval_gate(ctx: Any, config: dict[str, Any]) -> None:
    """Register the `pre_tool_call` hook that backs the iOS approval bubble.
    Defensive: missing `ctx.register_hook` (older Hermes) or a broken
    approvals module degrades to "no gate" rather than blocking plugin
    startup."""
    register_hook = getattr(ctx, "register_hook", None)
    if not callable(register_hook):
        logger.warning(
            "[onepilot] ctx.register_hook unavailable — approval gate disabled"
        )
        return
    if _PLUGIN_LOOP is None:
        logger.warning("[onepilot] plugin loop not captured — approval gate disabled")
        return
    try:
        try:
            from .approvals import register_pre_tool_call_hook
        except (ImportError, ValueError):
            import sys
            here = str(PLUGIN_DIR)
            if here not in sys.path:
                sys.path.insert(0, here)
            from approvals import register_pre_tool_call_hook  # type: ignore[no-redef]
    except Exception as exc:
        logger.warning(
            "[onepilot] approvals module import failed (%s) — approval gate disabled",
            exc,
        )
        return
    try:
        hook = register_pre_tool_call_hook(
            plugin_loop=_PLUGIN_LOOP,
            config=config,
            lookup_session_id=lambda: _lookup_session_id("default"),
        )
        register_hook("pre_tool_call", hook)
        # Hermes routes plugin logs to <profile>/logs/agent.log via
        # setup_logging(); the integration test greps for this line there.
        logger.info("[onepilot] pre_tool_call approval gate registered")
    except Exception as exc:
        logger.warning(
            "[onepilot] pre_tool_call hook registration failed: %s — approval gate disabled",
            exc,
        )
...
async def _onepilot_deliver(
    ...
    # Re-sign per attempt: the JWT exp is 90s, retries can outlive it.
    ...
    for attempt in range(len(delays) + 1):
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                r = await client.post(url, headers=_ingest_headers(), content=body_str)
...
async def _post_assistant_row(config: dict[str, Any], row: dict[str, Any], text: str, kind: str = "text") -> None:
    ...
    for attempt in range(len(delays) + 1):
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                r = await client.post(ingest_url, headers=_headers(), content=body_str)
            if r.status_code < 500 or attempt == len(delays):
                if r.status_code != 200:
...
async def _handle_user_message(config: dict[str, Any], row: dict[str, Any]) -> None:
    ...
    # Populate the (account_id, session_key) → session_id cache so the
    # synchronous `pre_tool_call` approval gate (running on the agent
    # thread) can broadcast on the right Realtime topic without a
    # backend round-trip. Hermes is single-account, so account_id is
    # always "default".
    _remember_session_id(
        "default",
        str(row.get("session_key") or config.get("sessionKey") or "main"),
        str(session_id),
    )

    # Intercept `/approve <id> <decision>` BEFORE history load / LLM
    # dispatch. Without this, the bare `/approve` text would be sent to
    # the model and the user's approval would never resolve the pending
    # threading.Event the agent thread is blocked on. Mirror of OpenClaw
    # `messaging.js:maybeHandleApproveCommand`.
    _approve_text = _extract_text(row.get("content"))
    _approve_match = APPROVE_REGEX.match(_approve_text or "")
    if _approve_match:
        approval_id = _approve_match.group(1)
        decision = _approve_match.group(2).lower()
        try:
            try:
                from .approvals import apply_decision, pending_snapshot
            except (ImportError, ValueError):
                from approvals import apply_decision, pending_snapshot  # type: ignore[no-redef]
            matched = apply_decision(approval_id, decision)
        except Exception as exc:
            logger.warning("[onepilot] approvals.apply_decision raised: %s", exc)
            matched = False
        id_short = approval_id[:16]
        if matched:
            confirm = f"✅ Approval {decision} submitted for `{id_short}`."
            logger.info(
                "[onepilot] /approve matched id=%s decision=%s", id_short, decision
            )
        else:
            try:
                ...
                pending = []
            confirm = (
                f"⚠️ Approval `{id_short}` was already resolved or expired."
            )
            logger.info(
                "[onepilot] /approve no pending entry id=%s pending_now=%s",
                id_short,
                pending,
            )
        try:
            await _post_assistant_row(config, row, confirm)
        except Exception as exc:
            logger.warning("[onepilot] /approve confirm post failed: %s", exc)
